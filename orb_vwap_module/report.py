...
def summarize(res: BacktestResult) -> dict:
    ...
    out = {
        ...
        "setups_armed": int(su["breakin_ts"].notna().sum()) if len(su) else 0,
        "breakins_failed_immediate": int(su["failed_attempts"].sum()) if len(su) else 0,
        "breakins_rejected_range": int(su["range_rejects"].sum()) if len(su) else 0,
...
def write_outputs(out_dir: str | Path, ab: dict[str, BacktestResult], grid: pd.DataFrame | None,
    ...
    lines += ["", "## Conteggio setup (identico per A e B: stesso motore di setup)", "",
              ...
              f"- Break-in scartati per lateralità insufficiente: {sA['breakins_rejected_range']}",
              f"- Break-in senza rientro+trigger sulla barra successiva (immediate_trigger): {sA['breakins_failed_immediate']}",
